        ...
        <nav class="z-50 w-full bg-[rgba(0,0,0,.8)] md:bg-[rgba(255,255,255,.9)] dark:md:bg-[rgba(17,24,39,.95)] block px-2 sm:px-4 py-2.5 fixed lg:shadow-md">
            <div class="container flex flex-wrap justify-between items-center mx-auto w-full">
                <NuxtLink v-bind:to="findMenuBySlug('about').value" class="flex items-center" draggable="false">
                   <img src="/images/moblan-logo.png" class="mr-3 w-[50px] h-auto" alt="Marx Logo" draggable="false" />
                   <span class="text-gray-400 font-bold hidden md:inline-block" v-text="quote"></span>
                </NuxtLink>
                <div class="flex items-center gap-2 ml-auto">
                    <!-- Theme Toggle -->
                    <button type="button" class="p-2 rounded-lg text-white md:text-gray-700 dark:md:text-gray-300 hover:bg-white/10 md:hover:bg-gray-100 dark:md:hover:bg-gray-700 transition-colors" :title="themeLabel" :aria-label="themeLabel" @click.prevent="cycleTheme">
                         <!-- System icon -->
                    ...
                    <template v-if="isOpen == false">
                         <button data-collapse-toggle="navbar-default" type="button" class="inline-flex items-center p-2 ml-3 text-sm text-white rounded-lg" aria-controls="navbar-default" aria-expanded="false" @click.prevent="isOpen = true;">
                ...
                <div class="w-full block w-auto shadow-[22px_2px_20px_0px_rgba(0,0,0,1)]" v-bind:class="{'open': isOpen == true, 'close': isOpen == false}" id="navbar-container">
